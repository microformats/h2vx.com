...
function strcat()
{ // takes as many strings as you want to give it.
    $strcatr = "";
    $isjs = js();
...
function numtosxgf($n, $f)
{
    ...
    return str_pad_left(numtosxg($n), $f, "0");
}
...
function sxgtonumf($s, $f)
{
    ...
    return str_pad_left(string(sxgtonum($s)), $f, "0");
}
...
function hxttonum($h)
{
    ...
    for ($i = 0; $i < $j; $i++) { // iterate from first to last char of $h
        ...
        if ($c >= 48 && $c <= 57) {
            ...
            $c -= 87;
        } // a-z case-insensitive treat as A-Z
        else {
            $c = 0;
        } // treat all other noise as 0
        $n = 36 * $n + $c;
...
function asintorsxg($a)
{ // ASIN to reversible sexagesimal; prefix ISBN-10 w ~
...
function auto_link($t)
{
    ...
    for ($i = 0; $i < $mlen; $i++) {
        ...
        // avoid double-linking, and don't link CSS @-rules
        if ((!$spe || !preg_match('/(?:ref|ata|src)\\=[\\"\\\']?/', $spe)) &&
            ...
            $t = strcat($t, '<a class="auto-link" href="',
                webaddresstouri($matchi, true), '">', $matchi, '</a>',
                $afterlink);
        } else {
            $t = strcat($t, $matchi);
        }
