<xsl:transform
	...
	<xsl:template name="mf:extractGeo">
		<xsl:choose>
			...
			<xsl:when test="local-name(.) = 'input' and @value">
				<xsl:variable name="longitude" select="normalize-space(substring-after(@value,';'))" />
				<xsl:variable name="latitude" select="normalize-space(substring-before(@value,';'))" />
				<xsl:variable name="altitude" select="0" />
				<!-- call display Function -->
				<xsl:element name="latitude">
					<xsl:value-of select="$latitude"/>
	...
	<xsl:template name="mf:extractDate">
		<xsl:param name="date-exclusive" select="0" />
		<xsl:param name="base-date"/>
		<xsl:param name="base-tzid"/>
		
		<xsl:choose>			
			<!-- if the property is on a TIME element check for @datetime -->
			<xsl:when test='local-name(.) = "time" and @datetime'>
					    ...
					    <xsl:for-each select=".//*[contains(concat(' ', normalize-space(@class), ' '),' value ')]"> 
					        <xsl:choose>
					        	...
					        	<xsl:when test='local-name(.) = "time" and @datetime'>
					        	    <xsl:call-template name="datetime:clock12-to-24">
                            	        <xsl:with-param name="time-string">
					        		        <xsl:value-of select="translate(normalize-space(@datetime), ':' ,'')"/>
				 	        	        </xsl:with-param>
        			                </xsl:call-template>
					        	</xsl:when>
					        	<!-- if the property is on an ABBR element check for @title -->
				        	    ...
				        	    -->
					        	<!-- if the property is on an INPUT element check for @value -->
					        	<xsl:when test='local-name(.) = "input" and @value'>
					        	    <xsl:call-template name="datetime:clock12-to-24">
                            	        <xsl:with-param name="time-string">
	...
	<xsl:template name="mf:extractUrl">
      ...
      <!-- if the property is on an INPUT element with value attribute, use it -->
      <xsl:when test='local-name(.) = "input" and @value'>
	...
	<xsl:template name="mf:class-attribute-value">
		...
		<xsl:for-each select=".//*[contains(concat(' ', @class, ' '), concat(' ', 'type', ' '))]">
			<xsl:choose>
				...
				<xsl:when test="local-name(.) = 'input' and @value">
					<xsl:if test="contains(translate(concat(' ', translate(@value,',',' '), ' '),$ucase,$lcase), concat(' ', $value, ' ')) = true()">
						<xsl:value-of select="normalize-space($value)"/>
					</xsl:if>
				</xsl:when>
